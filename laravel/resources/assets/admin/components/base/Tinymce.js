...
export default {
    ...
    methods: {
        ...
        async imagesUploadHandler(blobInfo, success, failure){
            ...
            const filesUrl = await fileUpload([blobInfo.blob()]);
            success(filesUrl);
        },
        async init() {
            let config = this.dcf;
            ...
            for (const p of config.plugins.split(' ')) {
                require('tinymce/plugins/' + p + '/plugin.min');
            }
            require('tinymce/themes/' + config.theme + '/theme.min');

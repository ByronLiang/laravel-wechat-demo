...
<script>
    import fileUpload from '../../api/qiniuUpload';

    export default {
        props: ['value', 'crop', 'max', 'disabled', 'multiple', 'limit', 'drive'],
        name: 'img-upload',
        data() {
            return {
                input: {
        ...
        methods: {
            ...
            async onChange(event) {
                ...
                const filesUrl = await fileUpload(event.target.files, this.options, {
                    onUploadProgress: (e) => {
